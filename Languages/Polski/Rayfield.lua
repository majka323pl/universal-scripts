...
if not okRay or not Rayfield then
    warn("Nie dało się wczytać Rayfielda!")
    return
end
...
-- Rayfield Window & Tabs (original UI preserved)
...
-- Home buttons (original)
local function safeLoadURL(url)
    pcall(function() loadstring(game:HttpGet(url))() end)
end

HomeTab:CreateButton({Name = "Anty-Fling", Callback = function() safeLoadURL("https://raw.githubusercontent.com/majka323pl/universal-scripts/refs/heads/main/anty-fling") end})
HomeTab:CreateButton({Name = "Click TP Tool", Callback = function() safeLoadURL("https://pastebin.com/raw/4KF2WJ9u") end})
HomeTab:CreateButton({Name = "Fly", Callback = function() safeLoadURL("https://pastebin.com/raw/xuSMWfDu") end})
HomeTab:CreateButton({Name = "Infinity Jump", Callback = function() safeLoadURL("https://raw.githubusercontent.com/majka323pl/universal-scripts/refs/heads/main/infinity-jump") end})
HomeTab:CreateButton({Name = "NoClip", Callback = function() safeLoadURL("https://pastebin.com/raw/CnbYfaj1") end})
HomeTab:CreateButton({Name = "Speed Tool", Callback = function() safeLoadURL("https://raw.githubusercontent.com/majka323pl/universal-scripts/refs/heads/main/X7B0HPc2") end})
HomeTab:CreateButton({Name = "Game Script (for this place)", Callback = function()
...
-- Premium
if hasPremium then
    PremiumTab:CreateButton({Name = "Console (InfiniteYield)", Callback = function() safeLoadURL("https://raw.githubusercontent.com/EdgeIY/infiniteyield/master/source") end})
    PremiumTab:CreateButton({Name = "Fling", Callback = function() safeLoadURL("https://raw.githubusercontent.com/0Ben1/fe/main/obf_rf6iQURzu1fqrytcnLBAvW34C9N55kS9g9G3CKz086rC47M6632sEd4ZZYB0AYgV.lua.txt") end})
    PremiumTab:CreateButton({Name = "Invisibility", Callback = function() safeLoadURL("https://pastebin.com/raw/3Rnd9rHf") end})
    PremiumTab:CreateButton({Name = "Spectate", Callback = function() safeLoadURL("https://raw.githubusercontent.com/majka323pl/universal-scripts/refs/heads/main/spectate") end})
else
...
- Nowa zakładka Update Logs
- Dodano Executor (bez premium)
- Usunięto Keysystem
...
-- helper: ustawia TextScaled = true dla elementów tekstowych
...
local function createModal(root)
    ...
    overlay.BackgroundTransparency = 1 -- overlay niewidoczny
    ...
    modal.BackgroundTransparency = 0 -- modal widoczny (usuwa problem półprzezroczystego tła)
...
local function openEditModal()
    if not ExecutorGui then return end
    if not selectedFile then rNotify("Edit","Najpierw wybierz plik ziomek!",3); return end
    ...
    btnSave.MouseButton1Click:Connect(function()
        ...
        if newName == "" then rNotify("Edit","Nie podałeś nazwy stary!",3); return end
...
local function openRenameModal()
    if not ExecutorGui then return end
    if not selectedFile then rNotify("Rename","Najpierw wybierz plik ziomek!",3); return end
    local old = selectedFile
    local overlay, modal = createModal(ExecutorGui)
    ...
    btnSave.MouseButton1Click:Connect(function()
        local newName = input.Text or ""
        if newName == "" then rNotify("Rename","Nie podałeś nazwy stary!",3); return end
        if not newName:match("%.lua$") then newName = newName..".lua" end
        local oldPath = scriptsFolder.."/"..old
        local newPath = scriptsFolder.."/"..newName
...
local function openDeleteModal()
    if not ExecutorGui then return end
    if not selectedFile then rNotify("Delete","Najpierw wybierz plik ziomek!",3); return end
...
local function createExecutor()
    ...
    btnLoad.MouseButton1Click:Connect(function()
        if not selectedFile then rNotify("Load","Najpierw wybierz plik ziomek!",3); return end
        local path = scriptsFolder.."/"..selectedFile
        if isfile(path) then
            local cont = safeRead(path)
            if cont then
                codeBox.Text = cont
                rNotify("Load","Wczytano: "..selectedFile,2)
            else
                rNotify("Load","Błąd przy wczytywaniu!",3)
            end
        else
            rNotify("Load","Nie ma takiego pliku!",3)
            refreshFileListUI()
        end
    end)
    ...
    btnRefresh.MouseButton1Click:Connect(function()
        refreshFileListUI()
        rNotify("Refresh","Odświeżyłem listę skryptów!",2)
    end)
...
ExecutorTab:CreateButton({
    ...
    Callback = function()
        ...
        if not toggleCreated then
            ...
            roundify(btn, 6)
            btn.TextScaled = true -- dopasowanie tekstu w toggle
        ...
        rNotify("Executor","Executor odpalony!", 3)
    end
})
